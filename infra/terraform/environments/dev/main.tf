terraform {
  ...
  # S3 backend for Dev state management (supports both AWS and Azure resources)
  backend "s3" {
...
# AKS Module - Azure Kubernetes (Same as prod for consistency)
module "aks" {
  ...
  # Kubernetes configuration - Same as prod
  kubernetes_version = var.kubernetes_version
  node_vm_size       = var.aks_node_vm_size  # 16GB RAM, 4 vCPU
  
  # Autoscaling - Same as prod
